    ...
    for (let { name, description, images, id } of items) {
        ...
        playListItem.innerHTML = `
           <img
                src="${imageUrl}"
                alt="${name}"
                class="rounded mb-2 object-contain shadow justify-items-center"
            />
...
const updateIconsForPauseMode = (id) => {
    ...
    const playButtonFromTracks = document.querySelector(`#play-track-${id}`);
    if (playButtonFromTracks) playButtonFromTracks.textContent = "play_arrow";
};
...
const playTrack = (
    ...
    if (audio.src == previewUrl) {
        ...
        const nowPlayingSongImage =
            document.querySelector("#now-playing-image");
        ...
        track.innerHTML = `
            ...
            <section class="grid grid-cols-[auto_1fr] place-items-center gap-2">
                ...
                <article class="flex flex-col gap-2 justify-center">
                    <h2 class="text-base song-title line-clamp-1 text-primary">${name}</h2>
...
const fillContentForPlaylist = async (playlistId) => {
    ...
    coverElement.innerHTML = `
        ...
        <section>
            ...
            <p id="playlists-details">${tracks.items.length} songs</p>
        </section>`;
    const pageContent = document.querySelector("#page-content");
...
const onContentScroll = (event) => {
    ...
    const coverOpacity =
        100 -
        (scrollTop >= totalHeight ? 100 : (scrollTop / totalHeight) * 100);
    const headerOpacity =
    ...
    if (history.state.type == SECTIONTYPE.PLAYLIST) {
        ...
        if (coverOpacity <= 35) {
            ...
            playlistHeader.classList.remove(
                "sticky",
                "bg-black-secondary",
                "px-8"
            );
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    //     playlist: "37i9dQZF1DWVDCraF986xg",
    // };
